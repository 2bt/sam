...
unsigned char CreateTransitions()
{
	...
	while(1) {
		...
		length += phonemeLengthOutput[pos];

		speedcounter = length + phase2;
		phase3       = length - phase1;
		transition   = phase1 + phase2; // total transition?

		if (((transition - 2) & 128) == 0) {
            interpolate_pitch(pos, length, phase3);
            for (int table = 169; table < 175; ++table) {
                // tables:
                // 168  pitches[]
                // 169  frequency1
                // 170  frequency2
    ...
    return length + phonemeLengthOutput[pos];
}
