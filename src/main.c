...
void OutputSound() {
    ...
    SDL_PauseAudio(0);
    while (pos < bufferpos) SDL_Delay(100);
    SDL_CloseAudio();
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (argv[i][0] != '-') {
            ...
            if (strcmp(&argv[i][1], "wav")==0) {
                ...
                i++;
                SetPitch((unsigned char)min(atoi(argv[i]),255));
            } else if (strcmp(&argv[i][1], "speed")==0) {
                i++;
                SetSpeed((unsigned char)min(atoi(argv[i]),255));
            } else if (strcmp(&argv[i][1], "mouth")==0) {
                i++;
                SetMouth((unsigned char)min(atoi(argv[i]),255));
            } else if (strcmp(&argv[i][1], "throat")==0) {
                i++;
                SetThroat((unsigned char)min(atoi(argv[i]),255));
            } else {
                PrintUsage();
                return 1;
            }
    ...
    for (int i = 0; input[i] != 0; i++) input[i] = toupper(input[i]);
...
//    if (wavfilename != NULL) WriteWav(wavfilename, GetBuffer(), GetBufferLength());
//    else                     OutputSound();
    WriteWav("out.wav", GetBuffer(), GetBufferLength());
    OutputSound();
